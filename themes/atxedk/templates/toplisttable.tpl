...
{section name=i loop=$tl_rows}
	<tr class='{cycle name=ccl}'>
		...
		<td>
			{if $tl_rows[i].rank}{$tl_rows[i].rank}.&nbsp;{/if}{if isset($tl_rows[i].subname)}{/if}{if $tl_rows[i].uri}<a class='kb-shipclass' href="{$tl_rows[i].uri}">{$tl_rows[i].name}</a>{else}{$tl_rows[i].name}{/if}{if isset($tl_rows[i].subname)}{/if}
