...
//		$this->_result = $this->CallAPI( "corp", "CorporationSheet", array( 'corporationID' => $myKeyString['corporationID'] ), $this->API_userID_, $this->API_apiKey_ );
		...
		$CorporationDetails = $CorporationApi->getCorporationsCorporationId($myKeyString['corporationID'], $EdkEsi->getDataSource());

		$tmpResults->allianceID = $CorporationDetails->getAllianceId();
		$tmpResults->corporationID = $myKeyString['corporationID'];
		$tmpResults->corporationName = $CorporationDetails->getName();
		$tmpResults->ticker = $CorporationDetails->getTicker();
		$tmpResults->ceoID = $CorporationDetails->getCeoId();
		$CharacterApi = new CharacterApi($EdkEsi);
		$CharacterDetails = $CharacterApi->getCharactersCharacterId($tmpResults->ceoID, $EdkEsi->getDataSource());
		$tmpResults->ceoName = $CharacterDetails->getName();
		$tmpResults->stationID = $CorporationDetails->getHomeStationId();
		$UniverseApi = new UniverseApi($EdkEsi);
		$UniverseDetails = $UniverseApi->getUniverseStationsStationId($tmpResults->stationID, $EdkEsi->getDataSource());
		$tmpResults->stationName = $UniverseDetails->getName();
		$tmpResults->description = $CorporationDetails->getDescription();
		$tmpResults->url = $CorporationDetails->getUrl();
		$tmpResults->taxRate = $CorporationDetails->getTaxRate();
		$tmpResults->memberCount = $CorporationDetails->getMemberCount();
		$tmpResults->memberLimit = '';
		$tmpResults->shares = $CorporationDetails->getShares();


		$AllianceDetails = $AllianceApi->getAlliancesAllianceId($tmpResults->allianceID, $EdkEsi->getDataSource());
		$tmpResults->allianceName = $AllianceDetails->getName();


		$this->_result = $tmpResults;
