...
class KillListTable
{
	...
	function generate()
	{
		...
		while ($kill  = $this->kill_list_->getKill())
		{
			...
			// Added BBK
				if((int) number_format($kill->getISKLoss(), 0, "","") <= 0) {
					$kll["victimshipisk"] = number_format($kill->getISKLoss()/1000000000, 0, ".","") . "";
				} elseif((int) number_format($kill->getISKLoss(), 0, "","")>1000000000) {
					$kll["victimshipisk"] = number_format($kill->getISKLoss()/1000000000, 2, ".","") . "b";
				} elseif((int) number_format($kill->getISKLoss(), 0, "","")>1000000) {
					$kll["victimshipisk"] = number_format($kill->getISKLoss()/1000000, 0, ".","") . "m";
				} else {
					$kll["victimshipisk"] = number_format($kill->getISKLoss()/1000, 0, ".","") . "k";
				}
			// End added BBK
