...
{if !$config->get('killlist_alogo')}{assign var=width_victim value=$width_victim+33}{/if}
	<table class="kb-table kb-kl-table" style="margin-left: auto; margin-right: auto; text-align: left;" cellspacing="1">
		<tr class="kb-table-header">
			...
			<!-- Added BBK -->
			<td class="kb-table-header" style="width: 40px; text-align:center">Value</td>
			<!-- end added BBK -->
		</tr>
...
{/if}
			...
			<td class="kb-table-cell" style="width: 133px; vertical-align: middle;"> <!-- Removed 20px BBK -->
				<div class="no_stretch" style="width: 133px;"> <!-- Removed 20px BBK -->
	...
	{/if}
			...
			<td style="width: 140px" class="kb-table-cell"> <!-- Removed 20px BBK -->
			<div class="no_stretch" style="text-align:left; width: 140px; height:auto">{if $config->get('killlist_regionnames')} {$k.region}, {$k.system}{else}<b>{$k.system}</b>{/if} ({if $k.loss || $k.kill}{$k.systemsecurity|max:0|string_format:"%01.1f"}{else}<span style="color:{if $k.systemsecurity >= 0.5}green{elseif $k.systemsecurity < 0.05}red{else}orange{/if};">{$k.systemsecurity|max:0|string_format:"%01.1f"}</span>{/if})<br /></div> <!-- Removed 20px BBK -->
	{if $k.inv || $comments_count}
		...
		{/if}
				...
				<!-- Added BBK -->
				<td style="width: 40px" class="kb-table-cell" align="right"><b>{$k.victimshipisk}</b></td>
				<!-- end added BBK -->
			</td>
		</tr>
	{/section}
