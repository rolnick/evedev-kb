...
class SSOComments
{
	 ...
	 * @return string
	 */
	function getSSOComments($commentsOnly = false)
	{
		global $smarty;
		...
		while ($row = $qry->getRow()) {
                        $pilot = Pilot::lookup($row['name']);
                        if (!$pilot){
                            $avatar = 'https://imageserver.eveonline.com/Character/1_32.jpg';
                        } else {
                            $avatar = $pilot->getPortraitURL(32);
                        }
	...
	function bbencode($string)
	{
		...
		$string = str_replace(array('[b]', '[/b]', '[i]', '[/i]', '[u]', '[/u]', '[s]', '[/s]'),
				array('<b>', '</b>', '<i>', '</i>', '<u>', '</u>', '<s>', '</s>'), $string);
